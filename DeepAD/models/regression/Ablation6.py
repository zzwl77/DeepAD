...
class SingleTaskFusion(nn.Module):
    ...
    def forward_single(self, taskmap, left_heatmap, right_heatmap):
        ...
        heatmap = self.fuse_heatmaps(left_heatmap, right_heatmap, method=self.stereo_fusion)
        # Extract features
...
class TemporalFusion(nn.Module):
    ...
    def __init__(self, embed_dim, hidden_dim):
        ...
        self.short_term_fusion_modules = nn.ModuleList([BiGatedFusion(5, embed_dim, hidden_dim)])
        self.long_term_fusion = BiGatedFusion(1, hidden_dim, hidden_dim)
    ...
    def forward(self, x_list):
        ...
        return fused_feature * 30  # 移除单一维度
...
class NBModel(nn.Module):
    ...
    def forward(self, heatmaps, taskmaps, age_education):
        ...
        out = self.sequence_fusion([task_feature3])  # [batch_size, 1]
