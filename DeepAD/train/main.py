...
parser.add_argument('--hdf5_file', default='data.hdf5', type=str)
...
def main():
    ...
    # train_dataset = AD_Dataloader(root = args.dataset, data_list=ETdata_train, label=args.questionnaire, transform=transform)
    # trainloader = data.DataLoader(train_dataset, batch_size=args.train_batch, shuffle=True, num_workers=args.workers)

    # test_dataset = AD_Dataloader(root = args.dataset, data_list=ETdata_test, label=args.questionnaire, transform=transform)
    # testloader = data.DataLoader(test_dataset, batch_size=args.test_batch, shuffle=False, num_workers=args.workers)
    ETdata = pd.read_csv(os.path.join(args.dataset, args.datacsv + '_u.csv'))
    full_dataset = AD2_Dataloader(root = args.dataset, data_list=ETdata, label=args.questionnaire, transform=transform, hdf5_file=args.hdf5_file)
    ...
    # print('Best PLCC: {}, Best SROCC: {}, Best RMSE: {}'.format(best_plcc, best_srocc, best_rmse))
    ...
    # Run test for the best epoch and write the network output in the log
    # Ensure to use the model parameters from the best epoch
    if best_plcc > 0.65:
...
def train(trainloader, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (gazemaps, taskmaps, age_edu, targets, image_path) in enumerate(trainloader):
...
def test(testloader, model, criterion, epoch, use_cuda, evaluate=False):
    ...
    with torch.no_grad():
        for batch_idx, (gazemaps, taskmaps, age_edu, targets, image_path) in enumerate(testloader):
            # measure data loading time
            data_time.update(time.time() - start_time)
